...
module%test [@name "file pickers"] _ = struct
...
module%test Typed = struct
  module%test Record = struct
    ...
    let%expect_test "calling [form_of_field_fn] multiple times gives the same value back" =
      let module T = struct
        type t =
          { a : int
          ; b : string
        ...
        Form.Typed.Record.make
          (module struct
            ...
            let finalize_view { f } (local_ _graph) =
              ...
              in
              Vdom.Node.div
                [ label_and_attr "a1" a1; label_and_attr "a2" a2; label_and_attr "b" b ]
            ;;
          end)
      in
      let handle = Handle.create (form_result_spec [%sexp_of: T.t]) component in
  ...
  module%test Variant = struct
      ...
      let component =
        Form.Typed.Variant.make
          (module struct
            ...
            let finalize_view picker_view selected (local_ _graph) =
              let inner_view =
      ...
      let component (local_ graph) =
        Form.Typed.Variant.make
          (module struct
            ...
            let form_for_picker (local_ graph) =
              let picker =
      ...
      let component =
        Form.Typed.Variant.make
          (module struct
            ...
            let form_for_picker (local_ _graph) =
            ...
            let finalize_view (_ : unit Bonsai.t) foo_view (local_ _graph) =
    ...
    let%expect_test "optional variant" =
      ...
      let component =
        Form.Typed.Variant.make_optional
          (module struct
            ...
            let finalize_view (_ : unit Bonsai.t) variant_and_subform (local_ _graph) =
      ...
      in
      let handle = Handle.create (form_result_spec [%sexp_of: T.t option]) component in
      Handle.show handle;
      [%expect
        {|
        (Ok ())
...
module%test [@name "Form.Typed.Record.make_table"] _ = struct
  module T = struct
    type t =
      { a : int
      ; b : string
  ...
  let component =
    Form.Typed.Record.make_table
      (module struct
        ...
        let form_for_field
          : type a.
            a Typed_field.t -> local_ Bonsai.graph -> (a, Vdom.Node.t) Form.t Bonsai.t
          =
          fun typed_field (local_ graph) ->
          match typed_field with
          | A -> Form.Elements.Textbox.int ~allow_updates_when_focused:`Never () graph
